...
class ShoppingTableViewController: UITableViewController {
    ...
    func listItems() {
        ...
        firestoreListener = firestore.collection(collection).whereField("authorID", isEqualTo: Auth.auth().currentUser!.uid).order(by: "quantity", descending: false).addSnapshotListener( includeMetadataChanges: true) { (snapshot, error) in
